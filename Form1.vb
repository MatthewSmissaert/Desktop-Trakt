...
Public Class Form1
    ...
    Private Sub ParsePhotoData(ByVal dataStream As Stream)

        'Dim sb As New StringBuilder
        'Dim serializer As New Newtonsoft.Json.JsonSerializer(GetType(Photos))
        'Dim photo As Photo
        'Dim photos As Photos = TryCast(serializer.ReadObject(dataStream), Photos)

        'If ((Not photos Is Nothing) AndAlso (Not photos.Items Is Nothing)) Then

        '    sb.Append(("Photo count=" & photos.Items.Count.ToString & Environment.NewLine))

        '    For Each photo In photos.Items
        '        If (Not photo.Media Is Nothing) Then
        '            sb.Append((photo.Media.Source.ToString() & Environment.NewLine))
        '        End If
        '    Next

        '    Debug.WriteLine(sb.ToString)
        'Else
        '    Debug.WriteLine(sb.ToString)p
        'End If
